...
var pindex = [];
var Materialplanid = '';
var url = '../../Controllers/Plan/Plan.ashx?action=editMaterialPlan';
//加载事件
$(function () {
    Materialplanid = GetQueryString("Materialplanid");
    $('#btnsave').click(function () {
        url = '../../Controllers/Plan/Plan.ashx?action=editMaterialPlan';
        工程名称save();
    });
    ...
    $.ajax({
        type: "POST",
        url: '../../Controllers/Plan/Plan.ashx?action=getMaterialPlan&Materialplanid=' + Materialplanid,
        dataType: 'json',
...
function save() {
    ...
    for (var i = 0; i < pindex.length; i++) {
        ...
        $("#td_" + pindex[i]).children("tr").each(function () {
            data += '{';
            var a = $(this).children();
            /// data += '{"Materialplanprojectdetailid":"1","Num":"' + (j * 100) + '","Amountmoney":"' + (j * 200) + '","Unitprice":"2" }';
    ...
    $.ajax({
        type: "POST",
        url:url,// '../../Controllers/Plan/Plan.ashx?action=editMaterialPlan',
        dataType: 'json',
        data: {
            "data": data,
...
function addTbody(index,obj)
{
    var tr = "<tr class='tr1'>" +
                     ...
                     "</ul>" +
                     "</div>" +
                     "<div id='table_gridView_'"+index+" class='dataTable' style='margin-top: 0.5em'>" +

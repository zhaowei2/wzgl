...
$(function () {
    $.ajax({
        type: "POST",
        url: '../../Controllers/Flow/Flow.ashx?action=getMyHandledTypeList',
        dataType: 'json',
...
function initGrid(index, type_code, definecode) {
    ...
    $.ajax({
        type: "POST",
        url: "../../Controllers/Flow/Flow.ashx?action=getMyHandledTask&TYPE_CODE=" + type_code + "&type=getCount&DEFINE_CODE=" + definecode + getWhere(),
        dataType: 'json',
        success: function (msg) {
            if (msg != '') {
                if (msg.IsSuccess == "true") {
                    ...
                    function pageselectCallback(page_index, jq) {
                        ...
                        $.ajax(
                        {
                            url: "../../Controllers/Flow/Flow.ashx?action=getMyHandledTask&TYPE_CODE=" + type_code + "&DEFINE_CODE=" + definecode + getWhere(),
                            type: "post",
                            data: {
                                "page": pageindex,
                                "rows": 15,
                            ...
                            success: function (data) {
                                ...
                                $.each(data.rows, function (key, val) {
                                    ...
                                    row += '  <span class="tabLiCon-text"><a href="' + val.URL + '?TASK_CODE=' + val.TASK_CODE + '&looktype=handle" >[' + val.DEFINE_NAME + ']' + val.TASK_TILTE + ' </a> </span>';
                                    row += '    </div>';
                                    ...
                                    row += '                 <span class="tabLiCon-status" title="查看"><a href="' + val.URL + '?TASK_CODE=' + val.TASK_CODE + '&looktype=handle" onclick="LookTask(\'' + val.TASK_CODE + '\')">' + '[' + stepname + ']</a></span>';
